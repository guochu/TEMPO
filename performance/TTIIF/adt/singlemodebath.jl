# ============================================================================
# 性能对比基准：XTRGIF vs TDVPIF
#
# 模型：independent bosons（费米杂质 + 单模玻色浴 DiracDelta），
#       用精确对角化（ED, ImpurityModelBase.correlation_2op_1τ/1t）作参考
#
# 运行方式：
#   OMP_NUM_THREADS=1 julia --project=. performance/XTRGIF/independentbosons.jl
#
# 比较内容：
#   1. 效率：影响泛函构建时间、观测量扫描时间、最终键维
#   2. 精度：虚时间 Green 函数 G(τ) 与实时间 greater Green 函数 G>(t)
#      相对 ED 参考解的相对误差，以及两种算法所得 IF 的相互偏差
#
# 初态约定（与 TEMPO 格点定义一致）：
#   - 虚时间轮廓：ED 用全相互作用哈密顿量的热态加权（correlation_2op_1τ）
#   - 实时间轮廓：ED 用非耦合初态 ρ_imp(最大混合) ⊗ 浴热态（correlation_2op_1t），
#     对应 TEMPO 实时格点的默认边界条件
# ============================================================================
...
# --------------------------- ED 参考解的构建 -------------------------------
# 全相互作用哈密顿量：H = ϵ_d n_d ⊗ I + I ⊗ ω₀ n_bath + n_d ⊗ (a' + a)
# 单模浴 Fock 空间截断为 nb（β=1、ω₀=1 时热平均占据 ~0.6，polaron 位移 ~1，
# nb=30 已远超收敛所需）
nb = 30
a = bosonaoperator(d=nb)
n_bath = bosondensityoperator(d=nb)
I2, Ib = one(nop), one(n_bath)

H_full = kron(ϵ_d .* nop, Ib) + kron(I2, Matrix(n_bath)) + kron(nop, a' + a)

op1 = [0.0 0; 1 0]   # d
op2 = [0.0 1; 0 0]   # d†
A1 = kron(op1, Ib)   # d 作用在杂质上
A2 = kron(op2, Ib)   # d† 作用在杂质上
...
# ============================================================================
println("="^70)
println("1. 虚时间基准 (imaginary time)")
println("="^70)
...
# ED 参考：相互作用热态 <d(τ) d†(0)>_β
exactGτ = correlation_2op_1τ(H_full, A1, A2, 0:δτ:(Nτ-1)*δτ, β=β)
...
# ED 参考：非耦合初态（杂质最大混合 + 浴热态）下的 <d(t) d†(0)>，
# 对应 TEMPO 实时格点的默认边界条件（ρ₀ = 最大混合态）
ρ0 = kron(Matrix(1.0 * I2 / 2), exp(-β .* Matrix(n_bath)))
exactG = -im .* correlation_2op_1t(H_full, A1, A2, ρ0, 0:δt:tmax)

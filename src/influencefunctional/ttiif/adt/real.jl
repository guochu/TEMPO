...
"""
	influenceoperator(lattice::RealADTLattice1Order, corr::RealCorrelationFunction, hyb::AdditiveHyb; algexpan=OverDeterminedProny())

`influenceoperator` method on real-time ADT lattices, returning a tuple of 4 branch MPOs ((+,+), (+,−), (−,+), (−,−)).
"""
function influenceoperator(lattice::RealADTLattice1Order, corr::RealCorrelationFunction, hyb::AdditiveHyb; algexpan::ExponentialExpansionAlgorithm=OverDeterminedProny())
	η⁺⁺, η⁺⁻, η⁻⁺, η⁻⁻ = _get_signed_corr(lattice, corr)
	op1, op2 = pairop(hyb)
	mpoj1 = adt_ti_mpotensor(η⁺⁺, op1, op2, algexpan)
	mpoj2 = adt_ti_mpotensor(η⁺⁻, fused_op(op1, :+), fused_op(op2, :-), algexpan)
	mpoj3 = adt_ti_mpotensor(η⁻⁺, fused_op(op1, :-), fused_op(op2, :+), algexpan)
	mpoj4 = adt_ti_mpotensor(η⁻⁻, transpose(op1), transpose(op2), algexpan)
	h1, h2, h3, h4 = _tompsj.(_get_mpo3(mpoj1)), _tompsj.(_get_mpo3(mpoj2)), _tompsj.(_get_mpo3(mpoj3)), _tompsj.(_get_mpo3(mpoj4))
...
"""
	influenceoperatorexponential(lattice::RealADTLattice1Order, corr::RealCorrelationFunction, dt::Real, hyb::AdditiveHyb, alg::TimeEvoMPOAlgorithm; algexpan=OverDeterminedProny())

`influenceoperatorexponential` method on real-time ADT lattices. `FirstOrderStepper` returns 4 MPOs and `ComplexStepper` returns 8 (one before and one after evolution for each branch).
"""
...
function influenceoperatorexponential(lattice::RealADTLattice1Order, corr::RealCorrelationFunction, dt::Real, hyb::AdditiveHyb, alg::ComplexStepper; 
	...
	h1a, h1b = _tompsj.(_get_mpo3(mpoj1a)), _tompsj.(_get_mpo3(mpoj1b))
	h2a, h2b = _tompsj.(_get_mpo3(mpoj2a)), _tompsj.(_get_mpo3(mpoj2b))
	h3a, h3b = _tompsj.(_get_mpo3(mpoj3a)), _tompsj.(_get_mpo3(mpoj3b))
	h4a, h4b = _tompsj.(_get_mpo3(mpoj4a)), _tompsj.(_get_mpo3(mpoj4b))
...
"""
	differentialinfluencefunctional(lattice::RealADTLattice1Order, corr::RealCorrelationFunction, dt::Real, hyb::AdditiveHyb, alg::TimeEvoMPOAlgorithm, algmult::DMRGAlgorithm; algexpan=OverDeterminedProny())

`differentialinfluencefunctional` method on real-time ADT lattices: multiply the branch differential IFs in successively to construct the full differential influence functional.
"""
...
function _fit_to_lattice_diag(lattice::RealADTLattice1Order, mpotensors, f1::Symbol, f2::Symbol, trunc::TruncationScheme=DefaultMPOTruncation)
	...
	data2[pos1] = ones(T, 1,d,1)
	data2[pos2] = ones(T, 1,d,1)
...
function _fit_to_lattice_offdiag(lattice::RealADTLattice1Order, mpotensors, f1::Symbol, f2::Symbol, trunc::TruncationScheme=DefaultMPOTruncation)
	...
	u_left, v_left = split_mpstensor(mpotensors[1], trunc)
	u_middle, v_middle = split_mpstensor(mpotensors[2], trunc)
	u_right, v_right = split_mpstensor(mpotensors[3], trunc)
...
band_boundary(lattice::RealADTLattice1Order, j::Int) = index(lattice, j, branch=:+), index(lattice, j, branch=:-)

function _get_signed_corr(lattice::RealADTLattice1Order, corr::RealCorrelationFunction)
...
function split_mpstensor(mpoj::DenseMPSTensor, trunc)
	d2 = size(mpoj, 2)
	...
	@tensor uu[1,2,4] := u[1,2,3] * ss[3,4]
	@tensor vv[1,3,4] := ss[1,2] * v[2,3,4]
	return uu, vv
end